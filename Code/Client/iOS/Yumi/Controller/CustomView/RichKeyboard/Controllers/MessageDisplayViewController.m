...
- (void)messageViewAnimationWithMessageRect:(CGRect)rect  withMessageInputViewRect:(CGRect)inputViewRect andDuration:(double)duration andState:(ZBMessageViewState)state{
    ...
    [self updateKeyboardHeight:self.messageToolView.height+rect.size.height];
...
- (void)initilzer{
    ...
    self.messageToolView = [[ZBMessageInputView alloc]initWithFrame:CGRectMake(0.0f,
                                                                               self.view.frame.size.height - inputViewHeight,width,inputViewHeight)];
    self.messageToolView.delegate = self;
    [self.messageToolView setTranslatesAutoresizingMaskIntoConstraints:YES];
...
- (void)shareFaceView{
    ...
    if (!self.faceView)
    {
        ...
        [self.faceView setTranslatesAutoresizingMaskIntoConstraints:YES];
...
- (void)shareShareMeun
{
    if (!self.shareMenuView)
    {
        ...
        [self.shareMenuView setTranslatesAutoresizingMaskIntoConstraints:YES];
